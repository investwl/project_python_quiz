        ...
        with psycopg.connect(conninfo="postgresql://postgres:angjaya08102005@127.0.0.1:5432/python_quiz") as connection:
            with connection.cursor() as cursors:
                for a in range(len(option_library_answer)):
                    if question_answer.upper() == option_library_answer[a][0]:
                        ...
                        for b in range(len(option_library_answer)):
                            cursors.execute("insert into option(question_id, option_label, option_answer) values('%s', '%s', '%s');"%(return_insert[0], option_library[b], option_library_answer[b]))
...
def add_question(userlogged):
                ...
                while True:
                    ...
                    while True:
                        ...
                        if len(fill_quiz) >= 2:
                            ...
                            fill_ans = fill_answer(fill_quiz, opening_quiz[1], get_user[0], option_library, question_answer)
                            break
                        else:
    ...
    return fill_quiz, opening_quiz[1], question_answer